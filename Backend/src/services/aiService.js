...
const GROQ_MODEL = "llama-3.3-70b-versatile"; // fast + free-tier friendly on Groq
...
export async function summarizeInvestigation(investigationData) {
  ...
  return callGroq(prompt, { maxTokens: 250 });
}
...
export async function explainAlert({ what, why }) {
  ...
  return callGroq(prompt, { maxTokens: 120 });
}
...
export async function summarizeReport(reportData) {
  ...
  return callGroq(prompt, { maxTokens: 200 });
}