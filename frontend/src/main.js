...
async function loadEntityProfileExtras(entityId) {
  ...
  try {
    ...
    const entityB = entities.find((e) => e.id === top.entityId);
    confidenceEl.textContent = `${top.confidence}% confidence`;
    bodyEl.innerHTML = `<div class="match-compare"><div><span>ENTITY A</span><strong>${escapeHtml(entityId)}</strong><small>${escapeHtml(entityA?.aliases?.[0] || "")}</small></div><div class="match-score"><b>${top.confidence}%</b><span>potential match</span></div><div><span>ENTITY B</span><strong>${escapeHtml(entityB?.id || top.entityId)}</strong><small>${escapeHtml(entityB?.aliases?.[0] || "")}</small></div></div><div class="reason-chips">${top.reasons.map((r) => `<span>${escapeHtml(r)}</span>`).join("")}</div>`;
